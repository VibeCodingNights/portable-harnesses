...
run_cell() {
  ...
  python run.py --task "${task}" --model "${model}" --adapter "${adapter}" \
...
print(f\"{r['task']:>4} | {r['model']:<10} | {r['adapter']:<11} | {mark:<9} | {r['tool_calls']:<10} | {r['errors']:<6} | {r['loops']:<5}\")
" 2>/dev/null || echo "  ${task} | ${model:<10} | ${adapter:<11} | ERR       | -          | -      | -"
}

print_header() {
  echo
  echo "Task | Model      | Adapter     | Completed | Tool Calls | Errors | Loops"
}
